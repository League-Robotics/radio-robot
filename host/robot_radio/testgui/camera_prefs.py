...
from ``_PROJECT_ROOT``). ``host/robot_radio/testgui/camera_prefs.py`` sits at
the same directory depth as ``host/robot_radio/config/robot_config.py``
(``host/robot_radio/<pkg>/<module>.py``), so the same four-``.parent`` chain
...
# host/robot_radio/testgui/camera_prefs.py -> repo root (same depth as
# host/robot_radio/config/robot_config.py's _PROJECT_ROOT).
