...
- [x] `Drive`'s constructor (`Drive.h:56-60`, `Drive.cpp:26-40`) changes its
...
- [x] Every STEP-5 read call site in `Drive::tickUpdate()` resolves through
...
- [x] `Drive::capabilities()`'s `caps.onboard_position = _otos.is_initialized();`
...
- [x] `Robot.cpp`'s `drive(...)` construction call passes `hal` instead of
      `hal.otos()` (`Robot.h` already declares `Hardware& hal;` as a member,
      so this is a bare identifier swap, no new member needed).
- [x] **All THREE ctypes test-harness files that construct `subsystems::Drive`
...
- [x] A new sim test (built on ticket 001's bench-otos substrate): drive,
      ...
      expected rigor).
- [x] Toggling `DBG OTOS BENCH 0` (back off) similarly restores the
      previously active sensor's readings on the live path on the next tick.
- [x] No behavior change for any session that never toggles bench mode — the
