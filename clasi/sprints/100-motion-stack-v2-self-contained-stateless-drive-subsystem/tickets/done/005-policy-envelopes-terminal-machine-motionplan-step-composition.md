...
- [x] `StepState{dwellStart, sustainStart, lastReplan, replanCount,
...
- [x] Replan envelopes implement the issue's table exactly: `e_along`
...
- [x] Terminal machine implements the issue's spec exactly: `t >= T_plan`
...
- [x] Flying handoff implements the issue's spec: exhausted AND `e_cross
      ...
      the right time.
- [x] Pose-fix step handling: `StepInput.poseStep`/`poseStepTheta`
...
- [x] `MotionPlan::step(const StepInput&, StepState*) const` composes:
...
- [x] `TrackRecord` carries every field from the issue's sketch (`in`,
...
- [x] Purity/determinism test: the same `(plan, StepInput, StepState)`
      fed to `step()` twice produces byte-identical `StepOutput` and
      resulting `StepState`.
- [x] Closed-loop scenario tests (against ticket 004's tracker + a plant
