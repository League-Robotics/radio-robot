...
- [ ] `src/host/robot_radio/DESIGN.md`'s `io/` row re-audited: it
      ...
      all — add one, noting ticket 002's loud-gate this sprint).
- [ ] `io/sim_loop.py:1073-1078`'s `step()` docstring ("50ms sim-time
      each") fixed to 40 ms (matching `_CYCLE_DURATION_S = 0.040`).
- [ ] `src/firm/app/robot_loop.cpp:502-503`'s trailing whitespace removed.
- [ ] Unit-suffixed parameters fixed per `.claude/rules/coding-standards.md`:
...
- [ ] Landmine annotations added: `kinematics/differential_drive.py`
      ...
      it never updates `_last_sync_s` and samples span reboots.
- [ ] Small decisions recorded (one line each, in this ticket or the
      ...
      gets a "no frames in N s" staleness banner.
- [ ] **The line-sensor dead-parity-tick item is explicitly deferred, NOT
      ...
      ticket) rather than fixing or silently dropping it.
- [ ] Each checkbox in `doc-rot-and-minor-sweep-from-2026-07-30-review.md`
      itself is marked done or struck with a stated reason.
