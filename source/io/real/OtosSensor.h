...
public:
    ...
    // 046-006: 3-DOF velocity read.  Reads the REG_VELOCITY_XL registers and
    // returns all three body-frame components: vx (forward), vy (lateral), and
    // omega (yaw rate), applying the same flip/rotation transform as
    // readVelocityTransformed.  Returns false on I2C error.  Unconditional —
