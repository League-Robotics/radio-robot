...
**Wheel control generations (128 ticket 015; superseded 130-005).** Four
generations of the wheel velocity-control law have existed in this
codebase; this section tracks which of them an engineer debugging
misbehaving wheels should suspect:
...
2. `Motion::WheelPid`/`Planner::stageDuty()` (`planner/wheel_pid.{h,cpp}`,
   ...
   cutover — see this file's own §6 for the open-question entry. Sprint
   130's own planner-honesty pass (tickets 007-009) is scheduled to delete
   this generation outright now that generation 4 below is the proven,
   shipped law — see that sprint's own Design Rationale.
...
| `planner/` | **`Motion::Planner`** — the on-robot motion decider (125–128, superseding the deleted `Motion::MoveQueue`): its own STANDALONE CMake project (`planner/CMakeLists.txt`), not built by this directory's own `CMakeLists.txt`. Profile generation (`profile.{h,cpp}`), jerk-limited shaping (`shape.{h,cpp}`), wheel-command estimation (`estimation.{h,cpp}`), a duty-stage closed loop (`wheel_pid.{h,cpp}`, PARKED from the live tick as of 128-015 — see "Wheel control generations" in §1), and the top-level `Planner` (`planner.{h,cpp}`) that ties them together and writes `Types::RobotState::Wheel::cmdVelocity` directly every cycle — no boundary interface, the blackboard field IS the boundary (128). `wheel_trim.{h,cpp}` (the velocity-domain trim loop that used to be summed into `cmdVelocity` here) is **DELETED outright (130-005)** — `Motion::Planner` publishes the bare profiled command now; the wheel-speed controller lives entirely in `App::Drive` (`src/firm/app/drive.{h,cpp}`) — see "Wheel control generations" in §1. `capi.cpp` builds the `motionplanner` shared library, the ctypes surface `src/tests/bench/planner_harness.py` drives. Its own `tests/` directory holds nine ctest executables (`profile_test`, `shape_test`, `estimation_test`, `planner_scenarios_test`, `planner_noise_test`, `ratio_lock_test`, `wheel_pid_test`, `planner_duty_scenarios_test`, `pose_ownership_test` — 128-016's own regression guard for `Odometry` being `state.pose`'s sole writer), run via the `planner_tests` custom target. See §4 below. |
...
- **RESOLVED (130-004/130-005, stakeholder 2026-08-01): the wheels' one
  ...
  stage stays PARKED (128-015) pending sprint 130's own planner-honesty
  pass (tickets 007-009), which is scheduled to delete `Motion::WheelPid`/
  `Planner::stageDuty()` outright now that generation 4 is the proven,
  shipped law (sprint 130 Design Rationale).
