...
leaves (`body_kinematics`, `odometry`, `state_estimator`, later
`wheel_velocity_pid`) and a larger `MoveQueue`/`WheelSink`/
`StopCondition`/`VelocityShaper` stack that was the ORIGINAL plan for how
...
| `wheel_velocity_pid.{h,cpp}` | `Motion::WheelVelocityPid` — the closed-loop velocity control law (125-003: relocated from `src/firm/devices/velocity_pid.{h,cpp}`, `Devices::MotorVelocityPid`). Byte-identical `compute()` body; only the namespace and the `Gains` parameter type changed (a motion-local plain aggregate, not `Devices::Gains` — the isolation invariant in §3). `App::Drive` holds the interim instances for its own WHEELS-teleop path. |
| `planner/` | **`Motion::Planner`** — the on-robot motion decider (125–128, superseding the deleted `Motion::MoveQueue`): its own STANDALONE CMake project (`planner/CMakeLists.txt`), not built by this directory's own `CMakeLists.txt`. Profile generation (`profile.{h,cpp}`), jerk-limited shaping (`shape.{h,cpp}`), wheel-command estimation (`estimation.{h,cpp}`), a duty-stage closed loop (`wheel_pid.{h,cpp}`) plus trim (`wheel_trim.{h,cpp}`), and the top-level `Planner` (`planner.{h,cpp}`) that ties them together and writes `Types::RobotState::Wheel::cmdVelocity` directly every cycle — no boundary interface, the blackboard field IS the boundary (128). `capi.cpp` builds the `motionplanner` shared library, the ctypes surface `src/tests/bench/planner_harness.py` drives. Its own `tests/` directory holds nine ctest executables (`profile_test`, `shape_test`, `estimation_test`, `planner_scenarios_test`, `planner_noise_test`, `ratio_lock_test`, `wheel_trim_test`, `wheel_pid_test`, `planner_duty_scenarios_test`), run via the `planner_tests` custom target. See §4 below. |
| `CMakeLists.txt` | The standalone `motion_tests` build (Design Rationale Decision 4) for THIS directory's own four leaves (`body_kinematics`/`odometry`/`state_estimator`/`wheel_velocity_pid`) — plain CMake, no `libfirmware_host`, no ctypes, no Python. Builds a static `motion` library from those four `.cpp` files. As of 128-014 there is no standalone (non-pytest) ctest coverage for any of them attached to this target yet — see the file's own comment and §4 below. Separate from, and does not depend on, `planner/`'s own CMake project. |
...
trajectory controller does — out of current scope). `wheel_velocity_pid`
is a standalone control law with no collaborators of its own —
`App::Drive` constructs and drives its own instances for WHEELS teleop.
`planner/` is entirely self-contained: it does not link against or
depend on any of this directory's own four leaves (it has its own
...
directory's four leaves only.** A real CMake project at
...
code they exercised; `body_kinematics`/`odometry`/`state_estimator` are
...
coverage for any of the four remaining leaves has this target ready to
attach to.
...
- **`Motion::WheelVelocityPid::compute(target, measured, gains,
  velDeadband, dt)`** — see `wheel_velocity_pid.h`'s own doc comment.
...
- **`motion_tests` currently has zero attached ctest coverage** (see §4)
  — a real gap for a future ticket, not a silent regression: the four
