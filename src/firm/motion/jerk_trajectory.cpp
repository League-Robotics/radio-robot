...
// jerk_trajectory.h for the class-level design notes (solve modes, the
...
// source/motion/jerk_trajectory.cpp`, generalized so solveToRest() and the
// new solveToState() share one solvePositionControl() that also carries a
// target velocity (previously hardcoded to 0).
...
// default, NOT a literal 0 (which would forbid any acceleration change). A
// positive value passes straight through. A negative value is not a
// meaningful configuration; treated the same as the sentinel rather than
// fed to Ruckig unchecked.
...
void JerkTrajectory::configure(const msg::PlannerConfig& config, bool isRotational) {
  if (isRotational) {
    // Symmetric by construction: both directions are bounded by the SAME
    // yaw_acc_max, so accelBoundsForDirection()'s mirroring formula
    // collapses to an identical result regardless of direction -- no
    // separate rotational branch is needed anywhere in this class.
...
void JerkTrajectory::accelBoundsForDirection(bool positiveDirection, float* maxAccel,
  ...
  // (decelerating) are. Mirror the assignment for a negative-direction
  // solve so "accelerating"/"decelerating" keep meaning the same physical
  // thing regardless of travel direction.
...
bool JerkTrajectory::solvePositionControl(float targetPosition, float targetVelocity,
                                           float currentPosition, float currentVelocity,
  ...
  input.target_velocity = {static_cast<double>(targetVelocity)};
  ...
  // its target, or a targetVelocity pointing the wrong way) now fails
  // cleanly -- and a failed solve leaves the previous trajectory in place
  // (see the temp-solve note below) -- instead of producing Ruckig's
  // otherwise-correct time-optimal overshoot-and-return.
  if (positiveDirection) {
...
bool JerkTrajectory::solveToRest(float targetPosition, float maxVelocity) {
  return solveToState(targetPosition, 0.0f, maxVelocity);
}

bool JerkTrajectory::solveToState(float targetPosition, float targetVelocity,
                                   float maxVelocity) {
  return solvePositionControl(targetPosition, targetVelocity, lastPosition_, lastVelocity_,
                               lastAcceleration_, maxVelocity);
}

bool JerkTrajectory::retarget(float newRemaining) {
  // Re-baseline to 0 (class comment); seed velocity/acceleration from this
  // channel's OWN last sample, never the position. Reuses the remembered
  // target velocity (0 unless the tracking solve was a solveToState()).
  return solvePositionControl(newRemaining, targetVelocity_, /*currentPosition=*/0.0f,
                               lastVelocity_, lastAcceleration_, maxVelocity_);
}

bool JerkTrajectory::reanchor(float position, float velocity) {
  // Re-solve to the SAME target/targetVelocity this channel was already
  // tracking; seed from the CALLER-SUPPLIED position/velocity with
  // acceleration forced to 0 -- a deliberate exception to the normal
  // seeding contract (class comment).
  return solvePositionControl(target_, targetVelocity_, position, velocity,
                               /*currentAcceleration=*/0.0f, maxVelocity_);
}
