...
3. `Motion::WheelTrim` (`planner/wheel_trim.{h,cpp}`) — the velocity-domain
   ...
   **DELETED outright (130-005, wheel-speed-controller-moves-into-
   ...
   Decision 2).
4. `Control::DifferentialDrive`'s unified wheel-speed controller (`src/firm/control/differential_drive.{h,cpp}`,
   130-004/130-005; relocated out of `core/` and renamed from
   `Core::DifferentialDrive` by 136-006) — the CURRENT live law, and the only one actuating any
