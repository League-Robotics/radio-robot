/**
 * differential.cpp -- the two-wheel maps.
 *
 * Bodies are byte-for-byte the former BodyKinematics free functions
 * (src/motion/body_kinematics.cpp); see differential.h for the
...
void Differential::inverse(float v, float omega, float b,
...
void Differential::forward(float vL, float vR, float b,
                                     float& v_out, float& omega_out) {
    v_out     = (vR + vL) * 0.5f;
    omega_out = (vR - vL) / b;
}

void Differential::saturate(float vL, float vR,
...
void Differential::inverse(const Twist& twist,
                                     float wheels[]) const {
    inverse(twist.v_x, twist.omega, trackWidth_, wheels[0], wheels[1]);
}

void Differential::forward(const float wheels[],
                                     Twist& twist) const {
    float v, omega;
