...
// `Hal::I2CBus` to a pure interface (hal/i2c_bus.h);
...
// SimPlant -- not on I2CBus)"): read()/write() (the Hal::I2CBus
...
class SimPlant : public Hal::I2CBus {
 public:
  // addr/data/len only -- no repeated/preClear/postClear parameter. Those
  // three exist on Hal::I2CBus::write()/read() purely to schedule real-
  ...
  // ---- Hal::I2CBus overrides -- hook wrappers only, see file header ----
  ...
  // on Hal::I2CBus (architecture-update.md Decision 1). port: 1 = left
