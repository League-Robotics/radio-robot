...
// for the live-pushed CFG patch fields that already survive a wire CFG
// call -- MotorConfigPatch's gains/travel_calib, OtosConfigPatch's
// scale/offset fields -- persisted across a POWER CYCLE, WIPED across a
// FIRMWARE VERSION bump. Does not widen the live-tunable set -- this
...
// handleConfig()'s own existing merge) -- so persisting one shared copy
// would silently lose a legitimate per-side travel_calib divergence the
// instant one is ever pushed. `side` itself is bookkeeping only (LEFT for
// motorL, RIGHT for motorR) -- deserializeSnapshot() always stamps it; it
// is never read back out of the blob.
//
