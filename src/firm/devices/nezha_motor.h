...
//     interim instances this sprint (see drive.h's own header); the rate
//     argument (encoder freshness ~80ms bounds the loop's effective update
//     rate regardless of residency) settles that motor residency bought
//     nothing.
...
//     but degraded on real hardware relative to the pre-125-003 behavior:
//     the Nezha brick's 0x46 register refreshes far slower (~80ms) than
//     the loop's own cycle (~40ms), so a naive per-tick velocity will
//     alternate between a real delta and a stale zero on the ticks between
//     brick refreshes, rather than holding the last real sample. Ticket
//     004's App::WheelObserver replaces this with one principled
...
class NezhaMotor : public Motor {
 public:
  ...
  // recent tick() call. A degraded placeholder pending ticket 004's
  // App::WheelObserver, which will restore a real per-sample age (the
  // Nezha brick's own ~80ms refresh vs. the ~40ms loop).
