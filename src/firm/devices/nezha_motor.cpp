...
void NezhaMotor::tick(uint64_t nowUs)
{
    ...
    // own, much-shorter dt against the brick's ~80ms refresh) is DELETED,
    // not relocated -- see this file's own header for why this is a
    // disclosed interim, not a silent regression, pending ticket 004's
    // App::WheelObserver.
    if (hasLastTick_) {
