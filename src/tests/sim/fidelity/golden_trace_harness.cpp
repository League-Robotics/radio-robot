...
// the current Control::DifferentialDrive through the SAME command
...
class ProbeMotor : public Hal::Motor {
 public:
  ...
  void setNeutral(Hal::Neutral) override { staged_ = 0.0f; }
...
class ProbeClock : public Hal::Clock {
 public:
...
class ProbeSleeper : public Hal::Sleeper {
 public:
...
std::vector<TraceRow> runComparison(const SharedGains& g,
  ...
  for (float commandMmPerS : commandMmPerS) {
    ...
    // PREVIOUS cycle's published samples, because Hal::Motor is
