...
``src/firm/kinematics/differential.cpp``, ``src/firm/core/odometry.cpp``) with
...
# src/tests/sim/plant/test_plant.py -> plant -> sim -> tests -> repo root
...
def test_plant_files_carry_no_heading_wrap_logic():
    ...
    its own -- a grep for atan2/fmod-wrap/wrapAngle across src/tests/sim/plant/
    must find nothing."""
    import re
    ...
    assert not offenders, (
        "src/tests/sim/plant/ must carry no heading-wrap logic of its own "
        f"(Decision 3) -- found:\n" + "\n".join(offenders)
    )
