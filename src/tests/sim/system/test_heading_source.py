"""src/tests/sim/system/test_heading_source.py -- sprint 109 ticket 005's
own sim-level acceptance: DISTANCE-mode arcs/pivots end to end on the real
App::RobotLoop/SimPlant graph, plus App::HeadingSource's OTOS-first/
encoder-fallback policy and its telemetry visibility (SUC-002/SUC-004).

Compiles ``heading_source_harness.cpp`` together with the same full
HOST_BUILD dependency graph ``test_move_queue.py`` already compiles
(mirrors its exact shape).

    uv run python -m pytest src/tests/sim/system/test_heading_source.py -v -s
"""
...
# src/tests/sim/system/test_heading_source.py -> system -> sim -> tests -> repo root
...
_HARNESS_SRC = _SYSTEM_DIR / "heading_source_harness.cpp"
...
def test_heading_source_harness_compiles_and_passes(tmp_path):
    """Compile heading_source_harness.cpp + its full dependency graph; assert
    ...
    binary = tmp_path / "heading_source_harness"
    ...
    assert compile_result.returncode == 0, (
        "heading_source_harness.cpp / its dependencies failed to compile:\n"
    ...
    assert run_result.returncode == 0, (
        "heading_source_harness reported a scenario failure "
