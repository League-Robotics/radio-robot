"""src/tests/sim/system/test_pilot_distance_trim.py -- ticket 112-003's own
targeted clamp-authority check: the 087-009 non-regression guardrail
(SUC-007) for App::Pilot's new bounded linear position-feedback trim.

Compiles ``pilot_distance_trim_harness.cpp`` together with the same full
HOST_BUILD dependency graph ``test_heading_source.py``/``test_move_queue.py``
already compile (mirrors their exact shape).

    uv run python -m pytest src/tests/sim/system/test_pilot_distance_trim.py -v -s
"""
...
# src/tests/sim/system/test_pilot_distance_trim.py -> system -> sim -> tests -> repo root
...
_HARNESS_SRC = _SYSTEM_DIR / "pilot_distance_trim_harness.cpp"
...
def test_pilot_distance_trim_harness_compiles_and_passes(tmp_path):
    """Compile pilot_distance_trim_harness.cpp + its full dependency graph;
    ...
    binary = tmp_path / "pilot_distance_trim_harness"
    ...
    assert compile_result.returncode == 0, (
        "pilot_distance_trim_harness.cpp / its dependencies failed to compile:\n"
    ...
    assert run_result.returncode == 0, (
        "pilot_distance_trim_harness reported a scenario failure "
