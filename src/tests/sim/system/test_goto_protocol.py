"""src/tests/sim/system/test_goto_protocol.py -- ticket 135-005's own
acceptance proof: the sim-executable half of the goto issue's own
Verification section (clasi/sprints/135-go-to-navigator-in-the-motion-
library/issues/replaceable-go-to-moves-in-the-motion-library.md) --
end-to-end GO_TO over the real wire codec (both frames), and a
...
# src/tests/sim/system/test_goto_protocol.py -> system -> sim -> tests -> repo root
...
def test_goto_protocol_scenarios_pass(tmp_path):
    """Compile goto_protocol_harness.cpp + its full dependency graph;
    assert every GO_TO/Navigator sim-system scenario passes, and print its
    own trace."""
    ...
    binary = tmp_path / "goto_protocol_harness"
    ...
    compile_result = subprocess.run(
        [
            ...
            str(_REPO_ROOT / "src"),
            "-I",
            str(_MOTION_PLANNER_DIR),  # navigator.h's bare #include "planner.h"
            "-I",
            str(_SUPPORT_DIR),
            "-I",
            str(_PLANT_DIR),
    ...
    assert compile_result.returncode == 0, (
        "goto_protocol_harness.cpp / its dependencies failed to compile:\n"
    ...
    assert run_result.returncode == 0, (
        "goto_protocol_harness reported a scenario failure "
