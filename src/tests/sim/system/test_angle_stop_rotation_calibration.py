...
(``just build-sim`` or ``cmake --build src/firm/platform/host/build``) -- skips cleanly if
...
pytestmark = pytest.mark.skipif(
    ...
    reason="sim lib not built -- cmake --build src/firm/platform/host/build (or `just build-sim`)",
)
