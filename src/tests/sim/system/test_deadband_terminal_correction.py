"""src/tests/sim/system/test_deadband_terminal_correction.py -- sprint 114
ticket 005's own sim SYSTEM acceptance proof (deadband-compensation-small-
commands-must-produce-real-motion.md): a real terminal correction that
falls inside NezhaMotor's outputDeadband_-equivalent floor produces real
motion and completes promptly instead of stalling out to Motion::Executor's
own ~8s stopTimeBackstopMs() arrive-timeout.

Compiles ``deadband_terminal_correction_harness.cpp`` together with the same
full HOST_BUILD dependency graph ``test_pilot_distance_trim.py``/
``test_heading_source.py`` already compile (mirrors their exact shape).

    uv run python -m pytest src/tests/sim/system/test_deadband_terminal_correction.py -v -s
"""
...
# src/tests/sim/system/test_deadband_terminal_correction.py -> system -> sim -> tests -> repo root
...
_HARNESS_SRC = _SYSTEM_DIR / "deadband_terminal_correction_harness.cpp"
...
def test_deadband_terminal_correction_harness_compiles_and_passes(tmp_path):
    """Compile deadband_terminal_correction_harness.cpp + its full dependency
    graph; assert every scenario passes."""
    ...
    binary = tmp_path / "deadband_terminal_correction_harness"
    ...
    assert compile_result.returncode == 0, (
        "deadband_terminal_correction_harness.cpp / its dependencies failed to compile:\n"
    ...
    assert run_result.returncode == 0, (
        "deadband_terminal_correction_harness reported a scenario failure "
