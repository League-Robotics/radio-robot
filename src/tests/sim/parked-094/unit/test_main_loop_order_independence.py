...
real ``src/firm/subsystems/{drivetrain,sim_hardware,pose_estimator,
...
``src/firm/hal/sim/{physics_world,sim_motor,sim_odometer}.cpp``, and
``src/firm/hal/velocity_pid.cpp`` -- with ``-DHOST_BUILD`` only (this harness
...
# src/tests/sim/unit/test_main_loop_order_independence.py -> unit -> sim -> tests -> repo root
