...
Ruckig... retire Motion::VelocityRamp" decision; src/firm/motion/
...
hardcoded src/firm/motion/velocity_ramp.cpp path below is permanently stale.
...
Motion::VelocityRamp (src/firm/motion/velocity_ramp.{h,cpp}) -- the body-level
...
Compiles ``velocity_ramp_harness.cpp`` together with ``src/firm/motion/
velocity_ramp.cpp`` (its one, dependency-free, real dependency) against the
SAME ``src/firm/motion/velocity_ramp.h`` every ARM build compiles. Mirrors
...
# src/tests/sim/unit/test_velocity_ramp.py -> unit -> sim -> tests -> repo root
