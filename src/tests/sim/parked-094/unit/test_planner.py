...
out of src/firm/ entirely to source_parked/094/subsystems/ (codal.json's
recursive src/firm/ glob forced the move once Motion::VelocityRamp -- this
file's/planner_harness.cpp's own real compile dependency -- was deleted; see
src/tests/sim/parked-094/README.md and clasi/sprints/094-.../architecture-
...
src/firm/subsystems/planner.cpp path below is stale by design -- do not "fix"
...
Subsystems::Planner (src/firm/subsystems/planner.{h,cpp}) -- the goal-closure
...
character itself is covered end to end in ``src/tests/sim/unit/
test_mode_machine.py``.

Compiles ``planner_harness.cpp`` together with ``src/firm/subsystems/
planner.cpp`` and its two real dependencies, ``src/firm/motion/
velocity_ramp.cpp`` and ``src/firm/motion/stop_condition.cpp`` (all
dependency-free -- no MicroBit.h, no I2CBus), against the SAME
``src/firm/subsystems/planner.h`` every ARM build compiles. Mirrors
...
``src/firm/motion/jerk_trajectory.cpp``, the vendored ``libraries/ruckig/src/
...
# src/tests/sim/unit/test_planner.py -> unit -> sim -> tests -> repo root
...
# (src/sim/CMakeLists.txt) -- without it planner.cpp would resolve
