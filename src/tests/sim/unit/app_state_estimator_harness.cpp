...
// sprint 117 ticket 002 (SUC-057), Motion::StateEstimator
// (src/motion/state_estimator.{h,cpp}, moved from src/firm/app/ by 122-002).
// Proves: ZOH distance/velocity wheel extrapolation, ZOH body pose
// extrapolation (straight-line and rotating), per-peer valid semantics
// (false before the first update(), true after, independently for each
// wheel and the body), reset()'s body-only re-anchor (wheel peers
// untouched, basisTime/valid untouched), innovations() residual computation
// (even at weight 0, diagnostic only), and the v1 complementary-blend
// weight cases (0.0 pure encoder, 1.0 pure OTOS, intermediate proportional
// blend) -- including the staleness gate (a too-old or absent OTOS reading
// blends nothing and leaves innovations() unchanged).
//
...
// Motion::StateEstimator::Input directly (the same fields App::RobotLoop's
// real updateTlm()/applyOtosSample()/odom_.integrate() calls stage into
// frame_ each cycle, copied verbatim -- see robot_loop.cpp's own
// stateEstimator_.update() call site) rather than scripting a
// SimPlant/ScriptedI2CHook bus. 122-002: this harness no longer references
// App::Telemetry::Frame or any other App:: type at all (Motion::
// StateEstimator's own update() no longer takes one -- src/motion may not
// depend on app/). Compiled by test_app_state_estimator.py with
// -DHOST_BUILD against state_estimator.cpp alone -- no other .cpp
...
// --- StateEstimator::Input builder -- explicit field assignment (not
// aggregate braces) so a future field reorder can't silently swap two
// same-typed fields under this harness. 122-002: builds Motion::
// StateEstimator::Input directly -- this harness no longer references
// App::Telemetry::Frame at all (Motion:: may not depend on app/). ---------

Motion::StateEstimator::Input makeFrame(float encLeftPos, float encLeftVel, uint32_t encLeftTime,
...
void scenarioWheelZohExtrapolation() {
  ...
  Motion::StateEstimator estimator;
  Motion::StateEstimator::Input frame = makeFrame(/*encLeftPos=*/100.0f, /*encLeftVel=*/50.0f,
                                           /*encLeftTime=*/1000,
...
void scenarioBodyZohStraightLine() {
  ...
  Motion::StateEstimator estimator;
  Motion::StateEstimator::Input frame =
...
void scenarioValidBeforeAndAfterFirstUpdate() {
  ...
  Motion::StateEstimator estimator;
  checkFalse(estimator.wheelNow(Motion::Wheel::Left).valid, "wheelNow(Left) invalid before any update()");
  checkFalse(estimator.wheelNow(Motion::Wheel::Right).valid, "wheelNow(Right) invalid before any update()");
  checkFalse(estimator.wheelAt(Motion::Wheel::Left, 1000).valid, "wheelAt(Left, t) invalid before any update()");
  ...
  checkTrue(estimator.wheelNow(Motion::Wheel::Left).valid, "wheelNow(Left) valid after update()");
  checkTrue(estimator.wheelNow(Motion::Wheel::Right).valid, "wheelNow(Right) valid after update()");
...
void scenarioInnovationsComputedEvenAtWeightZero() {
  ...
  Motion::FusionWeights zeroWeights;  // headingOtos=0, omegaOtos=0 -- pure encoder output
  Motion::StateEstimator estimator(zeroWeights);
  ...
  Motion::Innovations innov = estimator.innovations();
...
void scenarioSetWeightsPureEncoderPureOtosAndBlend() {
  ...
  Motion::StateEstimator estimator;  // starts at default FusionWeights{} == 0.0/0.0
...
void scenarioStaleOrAbsentOtosBlendsNothing() {
  ...
  Motion::StateEstimator estimator(Motion::FusionWeights{/*headingOtos=*/1.0f, /*omegaOtos=*/1.0f,
                                                     /*staleness=*/50});

  // First: a genuinely fresh OTOS reading DOES blend and sets innovations().
  Motion::StateEstimator::Input freshFrame =
      makeFrame(0, 0, 1000, 0, 0, 1000, 0, 0, /*poseH=*/0.2f, 0, 0, /*twistOmega=*/0.5f,
                /*otosPresent=*/true, /*otosHeading=*/1.0f, /*otosOmega=*/2.0f,
                /*otosTime=*/1000);
  estimator.update(freshFrame, /*now=*/1000);
  Motion::Innovations afterFresh = estimator.innovations();
  ...
  Motion::StateEstimator::Input noOtosFrame =
  ...
  Motion::Innovations afterAbsent = estimator.innovations();
  ...
  Motion::StateEstimator::Input staleFrame =
  ...
  Motion::Innovations afterStale = estimator.innovations();
...
int main() {
  ...
  if (g_failureCount == 0) {
    std::printf("OK: all Motion::StateEstimator scenarios passed\n");
    return 0;
  }
  std::printf("FAILED: %d assertion(s) across the Motion::StateEstimator scenarios\n", g_failureCount);
  return 1;
}
