...
(``src/firm/app/deadman.{h,cpp}``).

Compiles ``app_deadman_harness.cpp`` together with ``src/firm/app/deadman.cpp``
and the TestSim::SimClock host-test fake (``src/sim/
sim_clock.cpp``) against ``src/firm/devices/clock.h`` (a pure interface,
...
# src/tests/sim/unit/test_app_deadman.py -> unit -> sim -> tests -> repo root
