...
@pytest.mark.xfail(
    ...
    reason=(
        ...
        "original 111-002 reorder-experiment reason below no longer "
        "matches reality -- this harness does not merely fail two "
        ...
        "telemetry work). 125-003's own 4 TLM command-surface scenarios, "
        "written into this harness before the breakage was noticed, could "
        ...
        "Planner rework remains open, unrelated, follow-up work -- not "
        "telemetry-emit-policy work, and not required for 125-006's own "
        "acceptance (which rests on the relocated, passing tests instead "
        "of this dead one).\n\n"
        "Original 111-002 reason (kept for history, no longer the active "
        "cause): robot_loop.cpp's own request/collect sequencing was a "
        "deliberate, temporary reorder (drive_.tick() hoisted to the top "
        "of cycle()) -- see "
