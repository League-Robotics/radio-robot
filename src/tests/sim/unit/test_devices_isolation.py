...
isolation invariant" -- mirrors sprint 100's ``src/firm/drive/`` discipline,
see clasi/sprints/100-.../tickets/002-.../ for the precedent):

    src/firm/devices/ may #include ONLY: its own headers, the C/C++ standard
...
``#include`` line under ``src/firm/devices/*.{h,cpp}`` (recursively, so later
...
(DB-002 onward) that adds real ``.cpp``/``.h`` files under ``src/firm/
devices/``.
"""
...
# src/tests/sim/unit/test_devices_isolation.py -> unit -> sim -> tests -> repo root
_REPO_ROOT = pathlib.Path(__file__).resolve().parents[4]
_DEVICES_DIR = _REPO_ROOT / "src" / "firm" / "devices"
...
def _devices_source_files():
    assert _DEVICES_DIR.is_dir(), f"src/firm/devices/ missing: {_DEVICES_DIR}"
    files = sorted(
...
def _find_violations(files):
    ...
    for path in files:
        ...
        for lineno, line in enumerate(
            ...
            violations.append(
                f"{rel}:{lineno}: {token} reaches outside src/firm/devices/ "
...
def test_devices_isolation_no_foreign_includes():
    """No src/firm/devices/*.{h,cpp} file may #include outside devices/ (isolation invariant)."""
    files = _devices_source_files()
    violations = _find_violations(files)
    assert not violations, (
        "src/firm/devices/ isolation invariant violated -- see "
