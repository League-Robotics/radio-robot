#!/usr/bin/env python3
"""src/tests/bench/profiled_motion_verify.py -- 106-006 (SUC-030) Phase 2: the
SAME profiled straight leg and profiled in-place turn ticket 006's sim
scenario (src/tests/sim/system/test_profiled_motion_sim.py) exercises, run for
...
What this script proves, per ticket 006's own Acceptance Criteria:
  1. Standing verification gate (`.claude/rules/hardware-bench-testing.md`):
     ...
     line/color fields at all -- `src/firm/messages/telemetry.h`) and are
  ...
  2. A profiled straight leg and a profiled in-place turn, each run through
     ...
     that tick) captured to `src/tests/bench/out/profiled_<leg>_<timestamp>.csv`
...
Usage:
    uv run python src/tests/bench/profiled_motion_verify.py
    uv run python src/tests/bench/profiled_motion_verify.py --port /dev/cu.usbmodem2121102
    uv run python src/tests/bench/profiled_motion_verify.py --distance 300 --angle-deg 60
    uv run python src/tests/bench/profiled_motion_verify.py --relay
...
# Deadman-expiry event bit (src/firm/app/telemetry.h: kEventDeadmanExpired = 1u << 0).
EVENT_DEADMAN_EXPIRED = 1 << 0
# Boot-time one-shot artifact (src/firm/app/telemetry.h: kFaultI2CSafetyNet = 1u << 0) --
