...
load step ran. Fixed in `src/firm/commands/dev_commands.cpp` (`isBoundPort()`
...
Usage:
    ...
    uv run python src/tests/bench/ratio_governor_curve.py --sync-gain 0.8
    uv run python src/tests/bench/ratio_governor_curve.py --sync-gain 0      # negative control
    uv run python src/tests/bench/ratio_governor_curve.py --left 200 --right 120 \\
        --load-schedule 40,0,-20,-40

    # legacy single-curve mode on the coupled pair itself
    uv run python src/tests/bench/ratio_governor_curve.py --disturb-port 0 \\
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--csv",
                   default=str(_REPO_ROOT / "src" / "tests" / "bench" / "out" / "ratio_governor_curve.csv"),
                   help="CSV output path")
    return p.parse_args()


# ---------------------------------------------------------------------------
# Small helpers (kept local per src/tests/bench/'s locked, standalone-script layout).
# ---------------------------------------------------------------------------
