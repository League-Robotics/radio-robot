...
What this script checks, per verb, using the SAME captured TLM/EVT stream:
  ...
  4. TURN/RT: heading accuracy against the 086/087 tolerance bars pulled
     directly from `src/tests/sim/unit/test_motion_commands_arc_turn.py`
...
Usage:
    uv run python src/tests/bench/bench_ruckig_motion_verify.py [--port PORT] [--mode direct|relay]
    uv run python src/tests/bench/bench_ruckig_motion_verify.py --only D,T,TURN,RT,G
"""
...
# src/tests/sim/unit/test_motion_commands_arc_turn.py (089-005/006 retune) —
