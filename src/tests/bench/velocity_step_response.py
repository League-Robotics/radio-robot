...
Usage:
    ...
    uv run python src/tests/bench/velocity_step_response.py \\
        --kp 0.0014 --ki 0.005 --kff 0.00135 --imax 0.3 --kaw 20.0

    # Boot-default gains (skip config(), just characterize what's shipped):
    uv run python src/tests/bench/velocity_step_response.py --no-config

    # A single speed:
    uv run python src/tests/bench/velocity_step_response.py --speeds 140
"""
...
def _args() -> argparse.Namespace:
    ...
    p.add_argument("--csv", default=str(_REPO_ROOT / "src" / "tests" / "bench" / "out" / "velocity_step_response.csv"))
    return p.parse_args()
