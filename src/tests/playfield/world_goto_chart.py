...
# v1 TLM frame — none of which the new src/firm/ dev-loop firmware exposes
...
# over (not in this ticket's locked src/tests/bench/ file list) — this file will
...
Run modes:
    ...
    uv run python src/tests/bench/world_goto_chart.py --auto --cycles 1 --image out.png

    # Safe pipeline check — pick target + draw markers + planned path, NO motion:
    uv run python src/tests/bench/world_goto_chart.py --auto --plan-only

    # Interactive — SPACE runs one full cycle (new target each press), q quits:
    uv run python src/tests/bench/world_goto_chart.py
"""
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--image", default="src/tests/bench/out/world_goto.png",
