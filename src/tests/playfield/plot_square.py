...
# tests_old/bench/plot_square.py: it plots a square_run.py CSV log whose
# columns (pose_x/y, otos_x/y, enc_l/enc_r) come from the old-tree v1 TLM
# telemetry frame, which the new src/firm/ dev-loop firmware does not emit
# (it only exposes DEV M/DT STATE — no world pose, no OTOS, no encoder-only
# odometry yet). Do not attempt to make this runnable against the new tree
# until those return.
...
Usage:
    uv run python src/tests/bench/plot_square.py
    uv run python src/tests/bench/plot_square.py --log host_tests/square_run_log.csv
"""
