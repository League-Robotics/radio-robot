"""src/tests/testgui/test_tour_stop.py — Tour/GOTO stop reactivation and Sim-mode
...
Direct read of ``src/host/robot_radio/testgui/__main__.py`` (``_stop_tour`` ~line
...
"src/tests/testgui/ rejoins testpaths" ticket).

``src/host/robot_radio/testgui/__main__.py``'s internals are closures with no
test seam (``_build_main_window()`` returns only ``(window, app)``). Per the
established pattern in ``src/tests/testgui/test_set_origin.py``, these tests
...
Run with:
    QT_QPA_PLATFORM=offscreen uv run pytest src/tests/testgui/test_tour_stop.py -v
"""
