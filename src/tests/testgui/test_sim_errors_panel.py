"""src/tests/testgui/test_sim_errors_panel.py -- ticket 085-006: Sim-Errors panel
verification. Ported from ``tests_old/testgui/test_sim_errors_panel.py``
(issue testgui-sim-error-profile-config; extended to the full SIMSET knob set
by ticket 069-007).

Covers:
- The ``sim_errors_group`` QGroupBox and its spin boxes -- the historical
  four (``sim_err_encoder_mm``, ``sim_err_slip_turn``,
  ``sim_err_otos_linear``, ``sim_err_otos_yaw``) plus the eleven 069-007
  knobs (see ``_NEW_SIM_ERR_SPIN_TO_PROFILE_KEY`` below) -- exist and are
  populated from ``sim_prefs.load_sim_error_profile()`` at window build.
...
- Clicking ``sim_errors_apply_btn`` saves the current field values via
  ``sim_prefs.save_sim_error_profile`` and, when connected to a (fake)
  ``SimTransport``, live-applies via ``transport.apply_error_profile``.

No production code change: this is a pure verification pass (this
sprint's own predicted "verification finds nothing to fix" outcome, per
...
Run with::
    ...
    QT_QPA_PLATFORM=offscreen uv run pytest src/tests/testgui/test_sim_errors_panel.py -v
"""
...
class TestSimErrorsPanelExistence:
    def test_group_and_spin_boxes_exist(self, qapp, monkeypatch, tmp_path):
        ...
        from PySide6.QtWidgets import QDoubleSpinBox, QGroupBox, QPushButton  # type: ignore[import-untyped]
    ...
    def test_spin_boxes_populated_from_defaults(self, qapp, monkeypatch, tmp_path):
        ...
        try:
            ...
            assert slip_spin.value() == pytest.approx(0.0)
        ...
        sim_prefs.save_sim_error_profile(
            {
                ...
                "otos_lin_drift": 2.0,
                # NOTE (085-006 API drift fix): the pre-rebuild file used
                # -1.5 here, assuming the old SIMSET wire protocol's
                # per-SECOND drift-rate units. The current
                # sim_err_otos_yaw_drift spinbox range is -0.05..0.05
                # rad/TICK (083-001 reconciled the ctypes ABI to a per-tick
                # additive term with a much smaller range) -- -1.5 silently
                # clamps to the spinbox's -0.05 minimum, which is what made
                # this test fail on first real run against the current tree.
                "otos_yaw_drift": -0.03,
            }
        )
...
class TestSimErrorsPanelVisibility:
    def test_visible_when_sim_selected(self, qapp, monkeypatch, tmp_path):
        ...
        from PySide6.QtWidgets import QComboBox, QGroupBox  # type: ignore[import-untyped]
        from robot_radio.testgui.__main__ import _build_main_window
    ...
    def test_apply_saves_all_new_knob_field_values(self, qapp, monkeypatch, tmp_path):
        ...
        try:
            new_values = {
                ...
                # NOTE (085-006 API drift fix): see the identical note in
                # test_spin_boxes_populated_from_persisted_file -- the
                # spinbox's real range is -0.05..0.05 rad/tick; -2.0 would
                # silently clamp.
                "sim_err_otos_yaw_drift": -0.02,
            }
    ...
    def test_apply_calls_live_apply_on_connected_sim_transport(
        ...
        class FakeConnectedSimTransport(transport_module.Transport):
            ...
            def firmware_version(self) -> "str | None":
                ...
                # `_state["transport"] = transport`, so the Apply button
                # handler below found no connected transport and never
                # called apply_error_profile() at all -- the actual prior
                # failure mode, not a missing button wiring.
                return "test-fake"
