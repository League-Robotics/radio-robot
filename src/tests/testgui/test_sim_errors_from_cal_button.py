"""src/tests/testgui/test_sim_errors_from_cal_button.py -- ticket 085-006: the
Sim Errors panel's "From Calibration" button (issue
testgui-sim-errors-from-calibration-button, ticket 070-004). Ported from
``tests_old/testgui/test_sim_errors_from_cal_button.py``.
...
No production code change: as of ticket 073-003, ``_on_sim_errors_from_cal()``
delegates to ``sim_prefs.resolve_calibration_defaults()``, which its own
docstring states was written to keep this EXACT test file's monkeypatch
point (``robot_radio.config.robot_config.get_robot_config``) working
unchanged -- confirmed here by direct run, not just by reading that comment.

Run with::

    QT_QPA_PLATFORM=offscreen uv run pytest src/tests/testgui/test_sim_errors_from_cal_button.py -v
"""
