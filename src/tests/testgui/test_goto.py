"""src/tests/testgui/test_goto.py -- ticket 085-003: Camera GOTO pursuit-loop
...
``_GotoRunner`` (``src/host/robot_radio/testgui/__main__.py``, ~line 1374) is a
...
- **Part B** drives a REAL ``SimTransport`` (the compiled sprint-084
  ``src/firm/`` firmware sim) because one acceptance criterion can only be
...
Run with::
    ...
    QT_QPA_PLATFORM=offscreen uv run pytest src/tests/testgui/test_goto.py -v
"""
...
#: Mirrors _GotoRunner.TRUTH_MAX_AGE_S (src/host/robot_radio/testgui/__main__.py).
