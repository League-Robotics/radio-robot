"""src/tests/unit/test_clock_sync.py -- 099-009 (SUC-005).

Unit tests for ``robot_radio.robot.clock_sync.ClockSync`` -- the NTP-style
host-side clock offset estimator ticket 099-009's aprilcam end-to-end
playfield script (``src/tests/playfield/pose_fix_convergence.py``) uses for its
PING clock-sync helper (architecture-update.md (099) D6: "host maps clocks
via PING t= + RTT/2"). All tests are fully offline -- no serial port, no
hardware; a fake monotonic clock and canned ``send_fn`` replies make the
math deterministic.

Ported (2026-07-12, ticket 099-009) from ``tests_old/simulation/unit/
test_clock_sync.py`` -- ``ClockSync`` itself moved into the new
``src/host/robot_radio/robot/`` tree verbatim during the sprint 077 greenfield
rebuild (it is still exported from ``robot_radio.robot.__init__``, see
``__all__``), but its own dedicated test file was never carried forward
into the new three-domain ``tests/`` layout, leaving it with zero LIVE
pytest coverage (``tests_old/`` is excluded from collection --
``pyproject.toml``'s ``norecursedirs``). This port is a straight copy
(verified field-for-field against the current ``clock_sync.py`` source --
no API drift since the rebuild) plus a new ``TestToRobotTime`` section
covering ``to_robot_time()`` (added by this ticket: the host->robot
inverse of ``to_host_time()``, needed to map a host-captured camera
observation's timestamp onto the robot's own clock before sending it as a
``PoseFix.t``).
...
- to_robot_time: the exact inverse of to_host_time, with and without skew;
  None before calibration.
...
class TestNoCalibraton:
    ...
    def test_to_robot_time_none(self) -> None:
        cs, _ = _make_cs()
        assert cs.to_robot_time(1000) is None
...
# ===========================================================================
# to_robot_time (the inverse of to_host_time) -- 099-009 addition
# ===========================================================================

class TestToRobotTime:
    """Verify to_robot_time() is the exact inverse of to_host_time()."""

    def test_to_robot_time_none_before_samples(self) -> None:
        cs, _ = _make_cs()
        assert cs.to_robot_time(1234) is None

    def test_offset_only_round_trips_with_to_host_time(self) -> None:
        cs, _ = _make_cs(start_s=0.0)
        # offset = (100+140)/2 - 110 = 10
        cs.record_ping(t0=100.0, t1=140.0, t_robot=110.0)
        assert cs.skew is None  # single sample -> offset-only path
        host_time = cs.to_host_time(500.0)
        assert host_time is not None
        assert cs.to_robot_time(host_time) == pytest.approx(500.0)

    def test_offset_only_direct_arithmetic(self) -> None:
        cs, _ = _make_cs(start_s=0.0)
        # offset = (0 + 40)/2 - 30 = -10 (host behind robot)
        cs.record_ping(t0=0.0, t1=40.0, t_robot=30.0)
        # to_robot_time(host) = host - offset = host + 10
        assert cs.to_robot_time(90.0) == pytest.approx(100.0)

    def test_skew_model_round_trips_with_to_host_time(self) -> None:
        cs, _ = _make_cs()
        A, B = 1.00002, 8000.0
        for i in range(15):
            t_r = float(i) * 3000.0
            host_mid = A * t_r + B
            cs.record_ping(t0=host_mid - 10.0, t1=host_mid + 10.0, t_robot=t_r)
        assert cs.skew is not None

        for t_r in [0.0, 5000.0, 30000.0, 100000.0]:
            host_time = cs.to_host_time(t_r)
            assert host_time is not None
            back = cs.to_robot_time(host_time)
            assert back is not None
            assert back == pytest.approx(t_r, abs=1e-6)

    def test_skew_model_direct_arithmetic(self) -> None:
        cs, _ = _make_cs()
        A, B = 1.0, 12345.0
        for i in range(10):
            t_r = float(i) * 1000.0
            host_mid = A * t_r + B
            cs.record_ping(t0=host_mid - 5.0, t1=host_mid + 5.0, t_robot=t_r)
        assert cs.skew == pytest.approx(1.0, abs=1e-6)
        # host_mid = a*t_r + b  =>  t_r = (host_mid - b) / a
        assert cs.to_robot_time(cs._skew_b + 500.0) == pytest.approx(500.0, abs=1e-3)
