"""src/tests/unit/test_gen_messages_no_getters.py — getter regression guard (080-001).
...
the checked-in `src/firm/messages/*.h` files, so a regression is caught the
...
# src/tests/unit/test_gen_messages_no_getters.py -> unit -> tests -> repo root
