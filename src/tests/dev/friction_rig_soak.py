...
`src/firm/hal/capability/motor.h`) against the real coupled friction rig
...
transcript output per run (`src/tests/bench/out/`, gitignored per this
...
1. **Hot-flip soak, A/B bracketed** (SUC-001/SUC-004): `--test-port`
   ...
   Two arms:
     - **control** (`dwell=0`) — the architecture's own definition of
       ...
       `|deltaPWM|` slew cap (064-002, `src/firm/hal/nezha/motor_slew.h`,
...
2. **Mid-motion / at-rest RESET guard check** (SUC-002): drives
   ...
   (`kRestVelocity`/`kRestTicksRequired`, `src/firm/hal/capability/
...
Usage:
    uv run python src/tests/bench/friction_rig_soak.py
    uv run python src/tests/bench/friction_rig_soak.py --port /dev/cu.usbmodem2121102 \\
        --test-port 3 --load-port 4 --duty 40 --flips 150
    uv run python src/tests/bench/friction_rig_soak.py --skip-soak   # reset-guard check only
"""
...
_OUT_DIR = _REPO_ROOT / "src" / "tests" / "bench" / "out"
