...
``src/host/robot_radio/robot/nezha.py``, ``nezha_state.py``,
``src/host/robot_radio/testgui/transport.py``'s ``_HardwareTransport``,
``src/host/robot_radio/io/cli.py``, and ``tests/playfield/world_goto_chart.py``
for that sweep.
...
1. **No ``SerialConnection`` in play at all** -- ``_binary_tlm_queue`` is a
   ...
   never owns:
     - ``src/host/robot_radio/calibration/linear.py``,
       ``src/host/robot_radio/calibration/angular.py`` -- use
       ...
       ``SerialConnection``.
     - ``src/host/robot_radio/testgui/transport.py``'s ``SimTransport`` -- uses
...
2. **The data itself is gone from the binary wire schema** --
   ``src/host/robot_radio/calibration/fit_sim_error_model.py``'s residual
...
``src/host/robot_radio/robot/protocol.py``) rather than either (a) silently
...
def parse_historical_tlm_line(line: str) -> "TLMFrame | None":
    ...
    deleted from ``src/host/robot_radio/robot/protocol.py`` -- see this
