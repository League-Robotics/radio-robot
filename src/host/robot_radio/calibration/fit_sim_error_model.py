...
``SIMSET`` registry (``src/firm/commands/SimCommands.cpp``'s ``kSimRegistry[]``)
...
``encpose=`` — see ``src/host/robot_radio/robot/protocol.py``'s ``TLMFrame``).
...
def split_recording(
    ...
    ``TLMFrame`` dataclass (``src/host/robot_radio/robot/protocol.py``), not a
...
def apply_params(sim: Any, params: dict[str, float]) -> str:
    ...
    ``src/host/robot_radio/testgui/transport.py``'s ``_apply_profile_to_sim()``,
...
def push_params(conn: Any, params: dict[str, float]) -> str:
    ...
    *conn* must expose either ``send_command(line) -> str`` (the
    ...
    elsewhere in ``src/host/robot_radio/testgui/transport.py``) — whichever is
...
def _repo_root() -> Path:
    # src/host/robot_radio/calibration/fit_sim_error_model.py -> repo root is
    # 4 parents up (calibration -> robot_radio -> host -> src -> repo root).
