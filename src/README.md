...
| `scripts/` | Codegen tooling run by `build.py`: `gen_messages.py` (protos → `firm/messages/*.h`), `gen_pb2.py` (protos → `host/robot_radio/robot/pb2/`), `gen_boot_config.py` (active robot JSON → `firm/config/boot_config.cpp`), `gen_version.py` (pyproject version → `firm/types/version_generated.h`). |
