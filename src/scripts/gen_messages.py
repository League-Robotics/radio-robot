#!/usr/bin/env python3
"""Generate src/firm/messages/*.h — C++11 POD headers from proto3 message definitions.
...
and emits one header per proto file to src/firm/messages/.
...
# header, src/firm/messages/layout_checks.h, chosen over inline-per-header
...
def _emit_layout_checks_header(struct_names: list) -> str:
    """Emit src/firm/messages/layout_checks.h: one static_assert per struct.
...
def _emit_layout_checks_source() -> str:
    """Emit src/firm/messages/layout_checks.cpp — the TU anchor above."""
...
def _build_field_table(struct_name: str, md, msg_map: dict, table_index_of: dict) -> list[dict]:
    ...
    src/firm/messages/*.h (union-member paths for oneof arms, the `_`-suffixed
...
def generate_headers() -> dict:
    ...
    writing to src/firm/messages/ — currently the getter regression-guard test
