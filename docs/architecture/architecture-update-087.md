...
# Architecture Update -- Sprint 087: Two-plane blackboard, synchronous-update loop, Configurator, and command-queue transport (greenfield)
...
sprint architecture, for current module-naming conventions), and direct reads
...
performed during this planning pass (2026-07-06/07) to confirm the design
issue's own grounding still holds.
...
No cycles: `Blackboard` has no outgoing edges (a passive data holder);
subsystems depend only on `Blackboard`'s queue/state **types** (via `tick()`
arguments), never on each other, the router, or the Configurator; the
Configurator's fan-out is 4 (`Drivetrain`, `PoseEstimator`, `Planner`,
`Hardware`) -- at the guideline's edge, justified in Step 6 Decision 4 as the
one deliberate exception the whole design is organized around; `Loop`'s
fan-out is unbounded by the 4-5 guideline because it is the composition root
(same status `main.cpp` already has today), not a peer module.
...
- **New: `Rt::Blackboard`** (`source/runtime/blackboard.h`) -- owns, as plain
  ...
  (`WorkQueue<Statement,16>`), `driveIn` (`Mailbox<DrivetrainCommand>`),
...
unit-testable without a wiring harness -- the sprint's stated goal.
...
| `source/subsystems/communicator.{h,cpp}` | **Unaffected in kind** -- still a statement *source* (`hasStatement()`/`takeStatement()`); its output now feeds `bb.statementsIn` instead of a single-latch seam, a wiring change at the loop, not a `Communicator` interface change. |
...
- **Sequencing dependency on sprint 086.** Sprint 086 is active on
  ...
  **This sprint's tickets should not begin execution (and its branch should
  ...
  concern with the design itself.
...
baked in as the resolved recommendation. Real type names from the current
tree; `Rt::` remains the runtime-layer namespace.
...
field lists, the subsystem tier's existing input-axis conformance
...
`architecture-034.md`'s staleness relative to current naming. Where this
document extends beyond what was re-verified this pass (the other four
`*State` structs, `dev_loop.h:63`'s `DevLoop` holder, the exact
`main.cpp`/`sim_api.cpp` line ranges), it defers to the design issue's own
2026-07-06 grounding pass rather than re-asserting unverified specifics.

**Design quality.** *Cohesion*: every module in Step 3 passes the one-sentence,
no-"and" test. *Coupling*: fan-out is low everywhere except the Configurator
(4, justified, Decision 4) and the Loop (unbounded, but it is the composition
root, not a peer). No subsystem includes another subsystem's header or the
Blackboard's aggregate type -- only the generic queue-template header and
`messages/*.h` (an explicit boundary rule stated in Step 3/Reference code,
preventing a subsystem from reaching into an unrelated cell). *Boundaries*:
the `Mailbox`/`WorkQueue` interface is two methods wide per read/write side;
`SET` validation stays synchronous and narrow (Decision 3). *Dependency
direction*: `commands/` (translators) -> `runtime/` (Blackboard/router/
Configurator, transport only) -> `subsystems/` (control) -> `messages/`
(data types) -- consistent, no reversal, no cycle (Step 4's dependency graph).
...
a queue's public interface. *Circular dependencies*: none (Step 4).
*Leaky abstractions*: guarded against explicitly by the "subsystems never
include `blackboard.h`" rule. *Speculative generality*: the queue taxonomy is
exactly two types (`Mailbox`, `WorkQueue`), chosen by payload semantics, not
a generic pub-sub framework built ahead of need.
...
7. **Sprint-086 merge gate** (Migration Concerns) -- confirm 086 is `status:
   done` and merged to `master` before this sprint's execution lock is
   acquired; the team-lead should verify this at the start of ticketing/
   execution rather than this planning pass assuming a specific completion
   date.
