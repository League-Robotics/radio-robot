...
- [ ] The `EKF` class in `test_ekf.py` is extended to match the T001 5-state API:
...
- [ ] All existing sprint-022 test classes (`TestPredictStraight`, `TestPredictTurn`,
...
**`TestPredictVelocity`:**
- [ ] After a straight predict with `dCenter=100, dt_s=0.1`, `v` is near 1000 mm/s
  and `omega` is near 0.
- [ ] P[3][3] grows by `q_v` after one predict from zero P (random-walk).
- [ ] P[4][4] grows by `q_omega` after one predict from zero P.
- [ ] P[0][3], P[0][4], P[1][3], P[1][4] remain 0 (block decoupling invariant).

**`TestUpdateVelocity`:**
- [ ] With `v` state at 500 mm/s and measurement at 300 mm/s (within Mahalanobis
  threshold), state moves toward 300 mm/s after one `update_velocity()`.
- [ ] P[3][3] decreases after a velocity update (uncertainty reduced).
- [ ] Position states `x`, `y`, `theta` are NOT changed by `update_velocity()`.

**`TestMahalanobisGating`:**
- [ ] `update_position` with a large innovation (e.g. 1000 mm from a 10 mm
  uncertainty filter) is rejected; `rejected_count` increments.
- [ ] `update_position` with a small innovation (within 1-sigma) is accepted.
- [ ] `update_velocity` with an outlier velocity is rejected; counter increments.
- [ ] After a rejection, state and covariance are identical to pre-update values.
...
- [ ] Test is named `test_setpose_rebaselines_encoder_prev` and has a clear
  comment explaining the bug.

**Golden vectors (Python/C++ parity):**
- [ ] A `TestGoldenVectors` class with at least two test cases:
  ...
  - **Golden vector 2:** Same for `update_position(x=10.0, y=5.0)` after a
    predict step with non-zero P.
- [ ] A comment above the golden vector tests documents: "These values are the
  source of truth. If the C++ EKF produces different values, the C++ is wrong."

**Replay harness:**
- [ ] A `replay_tlm_log(log_path)` function in `test_ekf.py` (or a separate
  importable module `tests/dev/ekf_replay.py`) that:
  ...
  - Supports three modes via a flags argument: `encoder_only`, `otos_position`,
    ...
    update is skipped.
- [ ] A small synthetic TLM log file is committed to `tests/dev/fixtures/` for
  deterministic replay testing. The log must contain at least: 20 TLM frames with
  encoder data, 5 frames with OTOS position, 1 frame simulating a camera fix
  (large pose jump followed by encoder-only continuation).
- [ ] A test `TestReplayHarness.test_replay_encoder_only_advances_x` confirms
...
- [ ] Notebook header cell documents purpose, required data file, and usage.
- [ ] Data cell loads the committed log file from `tests/dev/fixtures/`.
- [ ] Replay cell runs `replay_tlm_log()` in the three configurations.
- [ ] Plot 1: x-y trajectory overlay (encoder-only, +OTOS position, +OTOS velocity).
- [ ] Plot 2: position error vs time for each configuration.
- [ ] Plot 3: fused v and omega vs encoder-rate and OTOS raw velocity.
- [ ] Plot 4: P[0][0] (position uncertainty) and P[3][3] (velocity uncertainty) vs
  time, showing covariance shrinking on updates and growing between.
- [ ] Plot 5: theta error vs time — illustrating heading drift between camera fixes
  and correction at each fix.
- [ ] All five plots use `matplotlib`; notebook runs `Run All` without exceptions.
- [ ] A note cell below Plot 5 explicitly documents the heading-axis weakness:
...
**Full test suite:**
- [ ] `uv run --with pytest python -m pytest -v` passes with all new and existing
  tests green.
