...
def test_drive_produces_moving_telemetry(transport: SimTransport) -> None:
    ...
    097: drives via the binary-translated ``S`` verb (``DEV DT VW`` has no
    binary arm and never will -- the legacy ``DEV`` debug command family was
    retired along with the rest of the text plane; see
    ``binary_bridge.py``'s module docstring). ``send()`` enqueues the line
    for the tick-thread, which translates it to a binary
    ``CommandEnvelope{drive: DrivetrainCommand{wheels}}`` via
    ``binary_bridge.translate_command()`` and dispatches it through
    ``SimConnection.send_envelope()`` -- exercising the SAME translation
    path ``_HardwareTransport.send()`` uses. Telemetry itself is delivered
    by the tick-thread's continuously-armed binary stream
    (``conn.drain_binary_tlm()``), not by this command's own reply.
    ...
    Decision 6) — ``S`` commands ``Drivetrain`` directly and never stages a
    ``msg::PlannerCommand``, so it correctly reads ``mode=I`` throughout
    this drive even though the wheels are genuinely moving. This test only
    needs to confirm THAT telemetry moves, which ``vel=`` alone already
    proves.
    """
    frames = []
