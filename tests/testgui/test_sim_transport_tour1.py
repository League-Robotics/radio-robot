"""tests/testgui/test_sim_transport_tour1.py — the headless equivalent of
...
(``tests/_infra/sim/build/libfirmware_host.{dylib,so}``), reads
...
mirrors ``tests/testgui/test_sim_loop.py``'s own guard.

Run with::

    uv run python -m pytest tests/testgui/test_sim_transport_tour1.py -v
"""
...
def test_tour_shaped_sequence_via_direct_twist_calls_drives_and_closes(sim_transport):
    ...
    # tests/bench/data/tour_traces/tour_tour_1_*.json -- TOUR_1 is NOT a
...
def test_tour_1_runs_to_completion_with_finite_small_closure(sim_transport):
    ...
    for attempt in range(1, _MAX_TOUR_ATTEMPTS + 1):
        ...
        # Mirrors tests/bench/tour_bench_run.py's own bench-rig convention:
