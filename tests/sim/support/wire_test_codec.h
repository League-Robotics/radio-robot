...
// (source/messages/wire_runtime.h) -- the same primitives msg::wire's own
// generated decodeInto()/encodeInto() engine (source/messages/wire.cpp) uses
...
// emitPrimary()/emitSecondary(): ACKs ride Telemetry.acks_[], never a
// body_kind=OK/ERR ReplyEnvelope). NOT a general schema-walking engine --
// there is no FieldDesc/MessageTable machinery here, just a flat
...
// emitPrimary()/emitSecondary(): never both in the same emit() call, ACKs
// ride inside Telemetry.acks_[] rather than a separate body_kind=OK/ERR
// reply) -- kUnknown covers anything else (malformed armor, unrecognized
// bytes, a HELLO/PING plain-text reply this decoder is not meant to parse).
...
// exercise the ack-ring/correlation-id path.
