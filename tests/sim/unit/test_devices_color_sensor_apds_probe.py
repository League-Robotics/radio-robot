...
REAL ``source/devices/color_sensor.cpp`` and ticket 108-002's real
``Devices::I2CBus`` implementation, ``TestSim::SimPlant``
(``tests/_infra/sim/sim_plant.cpp`` -- itself named as this exact ticket's
consumer in its own header comment) plus SimPlant's own
``wheel_plant.cpp``/``otos_plant.cpp`` dependencies, with ``-DHOST_BUILD``,
against the SAME ``source/devices/`` headers every ARM build compiles.
...
# tests/sim/unit/test_devices_color_sensor_apds_probe.py -> unit -> sim -> tests -> repo root
...
# Matches every other tests/sim/{unit,plant,system} harness's own compiled
# standard.
_CXX_STANDARD = "c++20"
