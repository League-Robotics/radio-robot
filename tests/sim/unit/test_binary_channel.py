...
# -- `otos` stays a stub, and `pose_fix`'s own THIRD variant (neither reset
# nor zero_encoders set -- a genuine delayed camera fix) also stays a stub
# until 099-008.
# ===========================================================================


@pytest.mark.parametrize("kwargs,expected_field", [
    (dict(pose_fix=pb_drivetrain.PoseFix()), 7),
    (dict(otos=pb_odometer.OdometerCommand()), 8),
], ids=["pose_fix_neither_flag", "otos"])
