"""Off-hardware acceptance proof for ticket 089-002 (SUC-002/SUC-003/
SUC-004/SUC-005).

Compiles ``jerk_trajectory_harness.cpp`` together with
``source/motion/jerk_trajectory.cpp`` and the vendored Ruckig sources
...
mirroring ``test_ruckig_smoke.py``), runs the resulting binary, and asserts
it exits 0. Mirrors ``test_ruckig_smoke.py``'s/``test_velocity_pid.py``'s
...
provided seed (architecture-update.md (089) Decision 8).
"""
...
# tests/sim/unit/test_jerk_trajectory.py -> unit -> sim -> tests -> repo root
