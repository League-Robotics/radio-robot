...
Compiles ``measurement_ring_harness.cpp`` against ``source/devices/
measurement_ring.h`` and ``source/devices/interpolation.h`` -- both
header-only, no companion ``.cpp`` for either -- with ``-DHOST_BUILD`` for
consistency with every other tests/sim/unit harness, though neither header
...
# tests/sim/unit/test_measurement_ring.py -> unit -> sim -> tests -> repo root
