...
see `tests/sim/conftest.py`'s own file header) and asserts it exits 0.
...
(`source/devices/i2c_bus_host.cpp`, `queueWrite()`/`queueRead()`/
`txnCount()`/`errCount()`) -- `Devices::I2CBus` is now a pure interface
(`source/devices/i2c_bus.h`), with exactly two concrete implementations:
...
`tests/sim/plant/plant_harness.cpp` and `tests/sim/system/` for that
pattern, and `tests/testgui/test_sim_loop.py` for the equivalent pure-Python
...
`Devices::MicroBitI2CBus` (`source/devices/microbit_i2c_bus.{h,cpp}`), which
