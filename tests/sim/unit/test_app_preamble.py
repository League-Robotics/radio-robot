...
i2c_bus_host,clock_host}.cpp``) with ``-DHOST_BUILD``, against the SAME
headers every ARM build compiles. Mirrors ``test_app_drive.py``/
``test_devices_otos.py``'s exact shape: compile with the system C++
...
def test_app_preamble_harness_compiles_and_passes(tmp_path):
    """Compile App::Preamble + its Devices leaf dependencies + the harness; assert every scenario passes."""
    sources = [
        _HARNESS_SRC,
        _PREAMBLE_SRC,
        _NEZHA_MOTOR_SRC,
