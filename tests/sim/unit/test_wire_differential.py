...
***THIS IS THE CORRECTNESS GATE the `source/app/` tickets (004+) are built
on top of.*** It proves the self-written firmware codec (``source/messages/
wire_runtime.{h,cpp}`` + generated ``wire.{h,cpp}``) agrees with the host's
`google.protobuf`-backed reference (``host/robot_radio/robot/pb2/``)
