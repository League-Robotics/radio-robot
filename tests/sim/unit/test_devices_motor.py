...
Compiles ``devices_motor_harness.cpp`` together with the HOST_BUILD
implementations it needs (``source/devices/i2c_bus_host.cpp`` — the scripted
I2CBus fake from DB-003 — plus ``source/devices/velocity_pid.cpp`` and
``source/devices/nezha_motor.cpp``) against the SAME ``source/devices/``
...
real sleeps. Mirrors ``test_devices_i2c_bus.py``'s shape exactly: compile
...
def test_devices_motor_harness_compiles_and_passes(tmp_path):
    """Compile the Devices motor leaf + armor + PID sources and the harness; assert every scenario passes."""
    sources = [_HARNESS_SRC, _I2C_HOST_FAKE_SRC, _VELOCITY_PID_SRC, _NEZHA_MOTOR_SRC]
    for src in sources:
    ...
    assert compile_result.returncode == 0, (
        "devices_motor_harness.cpp / its Devices sources failed to compile:\n"
