...
Why `tests/playfield/`, not `tests/bench/` (see `tests/CLAUDE.md`'s
...
`host/robot_radio/robot/protocol.py`), never raw text, never lock-step
...
2. **PING clock-sync** (`clock_sync_burst()`): a burst of binary `ping`
   round trips, recorded into a `ClockSync` (`host/robot_radio/robot/
...
Usage
-----
    uv run python tests/playfield/pose_fix_convergence.py \\
        --port /dev/cu.usbmodem2121102

    # Safe pipeline check -- camera + clock-sync + geofence only, no motion,
    # no PoseFix send:
    uv run python tests/playfield/pose_fix_convergence.py --plan-only
"""
...
# tests/unit/test_pose_fix_convergence_pure.py without a robot or camera.
# ===========================================================================
