"""tests/unit/test_pose_fix_convergence_pure.py -- 099-009 (SUC-005).

Exercises ONLY the pure-math helpers in ``tests/playfield/
...
``tests/playfield/`` itself is NOT pytest-collected (``pyproject.toml``'s
``testpaths`` = ``tests/sim``, ``tests/unit``, ``tests/testgui`` only,
per ``tests/CLAUDE.md``'s three-domain split -- ``tests/playfield/`` is HITL
CLI tooling, not a pytest suite) -- this file lives under ``tests/unit/``
and reaches the script module by inserting ``tests/playfield`` onto
``sys.path`` directly, the SAME pattern ``tests/sim/unit/
test_pose_fix_end_to_end.py`` uses for its own sibling ``tests/_infra/sim``
...
_PLAYFIELD_DIR = _REPO_ROOT / "tests" / "playfield"
